...
\end{cases}$$
	1. 逆文档频率（Inverse Document Frequency, $IDF_{t}$）
...
1. 基于聚类的算法：IVF 系列（Inverted File）
...
1. 基于图的算法：HNSW（Hierarchical Navigable Small World）
...
1. 基于哈希的算法：LSH（Locality-Sensitive Hashing）
...
1. **来源唯一性约束** | 抑制幻觉 | 明确要求 LLM 仅使用【检索上下文】中的信息，禁止编造 |

2. **溯源强制化** | 提升可信度 | 要求每个结论都标注对应的 Chunk 元数据（来源、章节、时间） |

3. **上下文优先级引导** | 聚焦关键信息 | 强调优先使用重排后的 Top-K Chunk，后序 Chunk 仅作补充 |

4. **冲突消解规则** | 避免矛盾回答 | 明确多 Chunk 信息冲突时的处理逻辑（如「以发布时间最新的为准」） |
