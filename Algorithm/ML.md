...
-   **常用激活函数比较：**
    ...
    -   **Leaky ReLU：**
        ...
        解决ReLU的神经元死亡问题([实战中，我个人常用的参数是0.001量级])
...
HDBSCAN（Hierarchical Density-Based Spatial Clustering of Applications with Noise）是 DBSCAN 的层次化扩展。它的核心思想不是用单一阈值直接划分簇，而是先在不同密度尺度下建立层次结构，再选出最稳定的簇。
...
- 若 \(\operatorname{core}_k(x_i)\) 小，说明附近很快就能找到 \(k\) 个邻居，这里密度高
- 若 \(\operatorname{core}_k(x_i)\) 大，说明附近较稀疏


### 2. 互相可达距离

HDBSCAN 不直接使用原始距离 \(d(x_i,x_j)\)，而是定义：

$$
d_{mreach}(x_i,x_j) = \max \left\{\operatorname{core}_k(x_i),
\operatorname{core}_k(x_j), d(x_i,x_j) \right\}
$$
...
所以这个距离实际上在普通距离上加了一个“密度惩罚”。这一步很关键，因为它把“几何接近”变成了“在密度意义下接近”。

### 3. 用互相可达距离构造最小生成树

将每个样本点 \(x_i\) 看成图中的一个顶点，任意两点之间连一条边，边权定义为互相可达距离：

\[
w_{ij}=d_{\mathrm{mreach}}(x_i,x_j)
\]

$$d_{\mathrm{mreach}}(x_i,x_j)=\max\left\{\operatorname{core}_k(x_i),\,\operatorname{core}_k(x_j),\,d(x_i,x_j)\right\}$$

在这个加权图上，HDBSCAN 不直接保留所有边，而是求它的**最小生成树**（Minimum Spanning Tree, MST）：

\[
T=\arg\min_{T'} \sum_{(i,j)\in T'} w_{ij}
\]

其中 \(T'\) 是所有能够连接全部顶点、且不含回路的生成树之一。
...
- \(d_{\mathrm{mreach}}\) 小，表示连接紧密
- 所以 \(\lambda\) 大，表示密度高

接下来，HDBSCAN 会从高密度到低密度逐渐观察这棵树：

- 在很高的密度下，只有特别紧密的小团体还能连在一起
- 随着密度阈值降低，一些小团体会扩大
- 再往后，不同团体可能合并

**这一步就是 HDBSCAN 的“Hierarchical”来源。**


### 5. 簇是怎么选出来的

如果直接看层次树，会有很多层、很多候选簇。  
HDBSCAN 不会把每一层都当结果，而是计算每个簇的**稳定性**：

\[
Stability(C) = \sum_{x_i\in C}\bigl(\lambda_i-\lambda_{\mathrm{birth}}(C)\bigr)
\]

- \(\lambda_{\mathrm{birth}}(C)\)：簇 \(C\) 出现时的密度水平
- \(\lambda_i\)：点 \(x_i\) 在该簇中能保持多久对应的密度水平
...
那么它的稳定性就高。最终，HDBSCAN 选择**稳定性高的簇**作为结果；不能稳定存在的点群被舍弃，其中一些点会被标记为噪声。
...
这相当于只在**一个密度尺度**下聚类。  
所以如果数据里有的簇很密，有的簇很稀，单一 \(\varepsilon\) 往往不好用。
...
- `min_samples`：决定核心距离中的 \(k\)，控制“局部密度”定义
- `min_cluster_size`：一个簇最少要有多少点
- `metric`：距离度量方式
- `cluster_selection_method`：如何从层次树中选最终簇
