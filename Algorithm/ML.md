...
- 均方误差 (MSE - Mean Squared Error)常用于回归问题，对离群点敏感。$$J(\mathbf{w}) = \frac{1}{m} \sum_{i=1}^{m} (y^{(i)} - \hat{y}^{(i)})^2$$

- 交叉熵损失 (Cross-Entropy Loss)又称对数损失，是分类问题中最常用的损失函数，尤其适用于逻辑回归和神经网络。它衡量两个概率分布之间的差异。$$J(\mathbf{w}) = -\frac{1}{m} \sum_{i=1}^{m} \left[ y^{(i)} \log(\hat{y}^{(i)}) + (1 - y^{(i)}) \log(1 - \hat{y}^{(i)}) \right]$$对于多分类问题（Softmax回归），其一般形式为：$$J(\mathbf{w}) = -\frac{1}{m} \sum_{i=1}^{m} \sum_{j=1}^{K} y_j^{(i)} \log(\hat{y}_j^{(i)})$$其中 $K$是类别总数。

- KL散度损失 (Kullback-Leibler Divergence Loss)又称相对熵，用于衡量真实概率分布与模型预测概率分布之间的信息差异，刻画用预测分布近似真实分布的信息损耗，具备非负、非对称特性，常应用于分布对齐、知识蒸馏等场景。离散概率分布下的基础定义：$$D_{KL}(P || \hat{P}) = \sum_{i=1}^{n} P(i) \log\left( \frac{P(i)}{\hat{P}(i)} \right)$$机器学习批量训练场景下的损失形式：$$J(\mathbf{w}) = \frac{1}{m} \sum_{i=1}^{m} D_{KL}(P^{(i)} || \hat{P}^{(i)}) = \frac{1}{m} \sum_{i=1}^{m} \sum_{j=1}^{K} P_j^{(i)} \log\left( \frac{P_j^{(i)}}{\hat{P}_j^{(i)}} \right)$$其中 $P$ 代表真实概率分布，$\hat{P}$ 代表模型预测概率分布，$m$ 为样本数，$K$ 为分布维度/类别数。
...
-   **算法区别：**
    ...
    **标准 Adam + L2 正则：**
    ...
    （权重衰减被除以 $\sqrt{\hat{v}_t}$，自适应学习率削弱了正则化效果）

    **AdamW（解耦权重衰减）：**
    $$w_t = w_{t-1} - \alpha \frac{\hat{m}_t}{\sqrt{\hat{v}_t} + \epsilon} - \lambda w_{t-1}$$
    
    （权重衰减 $\lambda$ 直接与学习率 $\alpha$ 解耦，保持恒定衰减强度）

-   **算法步骤（差异仅在第6步）：**

    1-5步同 Adam（计算 $\hat{m}_t$ 和 $\hat{v}_t$）
...
-   **算法改进：**
    ...
    Nadam 改为：
    $$\frac{\beta_1 \hat{m}_t + (1-\beta_1) g_t}{\sqrt{\hat{v}_t} + \epsilon}$$
...
| 优化器 | 适用场景 | 学习率敏感度 | 推荐优先级 |
|--------|----------|--------------|------------|
| **SGD** | 简单凸优化、大规模数据、需要精细调参时 | 高 | ★★☆ |
| **SGD+Momentum** | 计算机视觉(CNN)训练、需要良好泛化时 | 中 | ★★★ |
| **RMSProp** | RNN、非平稳目标、强化学习 | 低 | ★★☆ |
| **AdaGrad** | 稀疏梯度数据(NLP嵌入)、不再常用 | 低 | ★☆☆ |
| **Adam** | 通用深度学习、快速原型验证 | 低 | ★★★ |
| **AdamW** | Transformer、BERT、GPT、大规模预训练 | 低 | ★★★★ |
| **Nadam** | 需要更快收敛的Adam替代方案 | 低 | ★★☆ |

### 实践建议：

1.  **快速实验/原型开发：** 使用 **Adam**（lr=1e-3）或 **AdamW**（lr=1e-3, weight_decay=0.01）

2.  **计算机视觉(CNN)：** 
    -   追求精度：**SGD + Momentum**（lr=0.01~0.1，配合 StepLR 衰减）
    -   快速收敛：**AdamW**

3.  **自然语言处理(NLP)/Transformer：** 优先使用 **AdamW**，它是目前 LLM 训练的标准配置

4.  **生成模型(GAN)：** 通常生成器用 **Adam**，判别器用 **SGD**（或都用 Adam 但不同 lr）

5.  **强化学习：** **RMSProp** 或 **Adam**（处理非平稳奖励信号）

6.  **学习率预热(Learning Rate Warmup)：** 使用 Adam/AdamW 训练大模型时，前几轮线性地从0增加到目标学习率，可显著稳定训练

### 公式对比总结

所有自适应优化器（AdaGrad, RMSProp, Adam, AdamW）都可以看作以下通用形式的变体：

$$w_{t+1} = w_t - \frac{\alpha}{\text{scale}_t} \cdot \text{momentum}_t$$

-   **scale_t：** 梯度幅度的累积（自适应学习率）
-   **momentum_t：** 梯度方向的累积（动量）
