...
## **第一阶段：基础与架构奠基 (Pre-2017 ~ 2017)**

从统计方法到神经网络，最终在 2017 年迎来颠覆性的 Transformer 架构。

1. **传统方法积累**​：N-gram 简单有效但受困于长距离依赖；NNLM(2003) 首次将词映射为稠密向量；Word2Vec & GloVe(2013) 让词表示成为 NLP 标配。
2. **RNN/LSTM/GRU**​：成为序列建模主流，能处理变长序列，但存在梯度消失/爆炸、无法并行化的瓶颈。
3. **Transformer (2017)**​：Google《Attention Is All You Need》带来转折——**自注意力机制**彻底解决长距离依赖，**并行化训练**大幅提升效率。它为一切大模型提供了基础设施。

## **第二阶段：预训练与规模缩放 (2018-2022) - "大力出奇迹"​**

Transformer 架构 + 海量数据 + 超大算力，开启了预训练时代。

1. **预训练范式确立**​：
   - **GPT-1 (2018)**​：生成式预训练 + 解码器自回归，证明了在无标注文本上预训练后微调的有效性。
   - **BERT (2018)**​：双向编码器 + 掩码语言模型，11 项任务 SOTA，引爆"预训练 + 微调"范式。
   - **路线之争**​：GPT（自回归） vs BERT（自编码），彼时 BERT 在理解任务更胜一筹。
2. **规模缩放与能力涌现**​：
   - **GPT-3 (2020)**​：1750 亿参数，涌现出**少样本/零样本学习**能力，证明了 Scaling Law 的有效性。
   - **InstructGPT / RLHF**​：引入人类反馈强化学习，让输出更符合人类偏好，成为 ChatGPT 的技术前身。
   - **T5**​：Google 将所有 NLP 任务统一为"文本到文本"。
   - **Chinchilla**、**Gopher**、**Switch Transformer** 等探索数据与计算的最佳配比。
3. **Decoder-Only 统一江湖**​：GPT 风格架构在生成和少样本学习上的绝对优势使其成为主流。

## **第三阶段：对齐与多模态 (2022-2023) - "ChatGPT 时刻"​**

模型从"强大"走向"有用"和"可用"，AI 全民化时代开启。

1. **ChatGPT (2022 年底)**​：基于 InstructGPT + RLHF，前所未有的对话交互体验，震惊世界。
2. **GPT-4 (2023)**​：多模态（理解图像），推理能力大幅提升，专业考试达人类水平。
3. **开源生态爆发**​：Meta 发布 LLaMA 权重，催生了整个开源社区——Vicuna、Alpaca 等基于 LLaMA 的微调模型大量涌现；BLOOM 等也在推动开放生态。
4. **多模态融合**​：模型不再局限于文本，CLIP、DALL-E、Stable Diffusion 在图文跨模态领域取得突破。

## **第四阶段：推理、开源与智能体 (2024 至今) - "从思考到行动"​**

焦点从单纯扩大规模转向更智能地使用规模，**推理能力、Agent 与开源生态**成为主旋律。

1. **推理模型 (Reasoning Models) 诞生**​：
   - **OpenAI o1/o3**​：引入思维链推理（Chain-of-Thought），模型回答前会长时间"思考"，在数学、编程、科学推理上达到甚至超越人类水平。
   - **DeepSeek-R1**​：通过纯强化学习激发推理能力，开源权重，证明 RL 不仅是对齐工具，更是**能力获取工具**。
   - Qwen-QwQ、Kimi k1.5 等纷纷跟进。
2. **开源模型全面崛起**​：
   - **LLaMA 3/4 (Meta)**​：400B 逼近 GPT-4，LLaMA-4 引入 MoE 架构。
   - **Qwen 2.5/3 (阿里)**​：0.5B 到 72B 全尺寸开源，多语言、长文本表现优异。
   - **DeepSeek-V3**​：671B MoE（37B 激活），训练成本仅 ~557 万美元，性能达 GPT-4o 级别，证明了极致工程优化的重要性。
   - Mistral Large 2、Gemma 2、Yi-Lightning 等持续丰富生态。
3. **多模态全面普及**​：
   - **GPT-4o**​：原生多模态（文本+图像+音频），实时交互延迟低至 200ms。
   - **Claude 3.5 / Claude 4**​：视觉理解大幅提升，Claude 4 Opus 混合模态推理创新高。
   - **Gemini 2.0 / 2.5 Pro**​：原生多模态 + 100 万 Token 上下文，Gemini 2.5 Pro 登顶 LMSYS。
4. **AI Agent 从概念走向产品**​：模型从"会聊天"进化为"能干活"——
   - **产品**：Claude Computer Use（操控桌面）、OpenAI Operator / Deep Research（自主调研）、Manus（通用任务执行）、Google Project Mariner / Astra。
   - **协议**：MCP（Anthropic，标准化工具连接，称"AI 界 USB"）、A2A（Google，Agent 间通信，迈向多智能体协作）。
   - **框架**：LangGraph（有状态编排）、CrewAI（多 Agent 协作）、Function Calling 成为所有模型的标配。
   - **趋势**：推理模型 + Agent 融合，产生"先思考再行动"的深度 Agent（如 Deep Research），任务成功率大幅提升。
5. **效率与长上下文**​：MoE（Mixtral、DeepSeek-V2/V3）成为主流架构；上下文窗口普遍来到 128K-1M Token；Phi-3/4 等小模型让端侧部署成为现实。
...
其中 $S_{t-1}^\top k_t$ 是当前状态根据键 $k_t$ 读出的旧值，$v_t-S_{t-1}^\top k_t$ 是预测误差。Delta Rule 可以先理解成一个**在线键值记忆更新规则**。
...
#### 3.2 WordPiece - BERT、DistilBERT 使用
...
#### 3.3 Unigram Language Model - SentencePiece (ALBERT、T5、LLaMA 后期使用)
...
#### 3.4 Byte-level BPE - GPT-2/4、Qwen2 使用
...
[Muon](https://github.com/KellerJordan/Muon)（MomentUm Orthogonalized by Newton-schulz）是一种主要用于神经网络**隐藏层矩阵权重**的优化器。它先像 SGD/Momentum 一样得到梯度更新，再把这个**更新矩阵**做正交化处理。

#### 1. 为什么要对更新矩阵做正交化

AdamW 对每个参数元素独立计算一阶、二阶统计量，再逐元素缩放梯度；这种方式适合通用参数，但没有显式利用二维矩阵的几何结构。对于线性层权重，更新矩阵的不同方向可能具有非常不同的尺度，容易让某些奇异方向主导更新。Muon 的思路是让更新的奇异值更加均衡，从而改善矩阵参数的整体更新方向。

注意：Muon 正交化的是 $\Delta W$，不是把 $W$ 本身约束成严格的正交矩阵，因此它不会简单地破坏模型已经学到的权重结构。

#### 2. 动量更新与极分解

设二维权重矩阵 $W_t\in\mathbb{R}^{m\times n}$，梯度为 $G_t=\nabla_W\mathcal{L}$，动量缓冲区为 $M_t$。Muon 先进行 Momentum 更新；部分实现还会使用 Nesterov 形式：

$$
M_t=\beta M_{t-1}+(1-\beta)G_t,
B_t=
\begin{cases}
(1-\beta)G_t+\beta M_t,\quad &\text{Nesterov}\\
M_t,\quad &\text{Momentum}
\end{cases}
$$
...
其极分解中的正交因子为 $UV^\top$。因此，Muon 的理想化更新可以写成：

$$
\mathrm{Orthogonalize}(B_t)\approx UV^\top,\qquad W_{t+1}=W_t-\eta\,\mathrm{Orthogonalize}(B_t)
$$

这里的“正交化”不是要求每个元素接近 0 或 1，而是让更新矩阵的奇异值不再被某几个方向严重拉大。实际实现不会显式执行 SVD，因为 SVD 对大矩阵代价较高。

#### 3. Newton--Schulz 近似正交化

官方实现使用固定步数的 Newton--Schulz 五次迭代近似矩阵的零次幂/正交因子。先对 $B_t$ 做归一化并在需要时转置，使参与计算的矩阵形状更适合矩阵乘法：

$$
X_0=\frac{B_t}{\lVert B_t\rVert_F+\epsilon}
$$

每一步先计算 $A_k=X_kX_k^\top$，再用五次多项式更新：

$$
A_k=X_kX_k^\top,\qquad X_{k+1}=aX_k+(bA_k+cA_k^2)X_k
$$

官方 Muon 实现中的一组系数为 $(a,b,c)=(3.4445,-4.7750,2.0315)$，通常执行少量固定步数（例如 5 步）。迭代结束后再恢复原始转置方向，并将结果作为更新矩阵。它的优点是只需要矩阵乘法，能够在 GPU/bfloat16 环境中高效运行。

#### 4. 完整参数更新

将正交化结果记为 $Q_t$，并考虑解耦权重衰减后，更新可概括为：

$$
Q_t=\gamma_{m,n}\,\mathrm{Orthogonalize}(B_t),\qquad W_{t+1}=(1-\eta\lambda)W_t-\eta Q_t
$$

其中 $\gamma_{m,n}$ 是与矩阵长宽比有关的缩放因子，$\lambda$ 是权重衰减系数。Muon 的学习率单位更接近“每次更新允许改变多少谱范数”，因此不能直接照搬 AdamW 的学习率。

#### 5. 参数分组与适用范围

- **适合 Muon 的参数**：Transformer 隐藏层中的二维线性权重，以及展平后的卷积核矩阵。
- **通常继续使用 AdamW 的参数**：输入 Embedding、最终输出头、分类头、偏置、LayerNorm/RMSNorm 的缩放参数等。官方参考实现明确采用 Muon + AdamW 的混合参数组。
- **工程注意**：需要重新调学习率、动量、权重衰减和 Newton--Schulz 步数；分布式训练还要考虑矩阵正交化和参数同步的额外开销。
...
| --- | --- | --- |
| 更新对象 | 逐元素更新 | 二维矩阵更新 |
| 梯度处理 | 一阶/二阶自适应缩放 | Momentum + 矩阵正交化 |
| 是否利用矩阵结构 | 基本不利用 | 显式利用奇异方向结构 |
| 参数覆盖 | 可覆盖全部参数 | 主要覆盖隐藏矩阵，其他参数通常交给 AdamW |
| 主要代价 | 维护一阶、二阶状态 | 额外矩阵乘法和分布式同步 |
...
4. 输入表示：Segment Embedding

原始 BERT 的输入表示由三部分相加得到：

$$
H_0=E_{token}+E_{segment}+E_{position}
$$

- **Token Embedding**：将每个 Token 映射为词向量。
- **Segment Embedding**（也叫 **Token Type Embedding**）：标记每个 Token 属于句子 A 还是句子 B。它不是语义上的“句子向量”，而是一个用于区分句子来源的可学习向量。
- 对输入 `[CLS] A [SEP] B [SEP]`，通常将句子 A 及其 `[SEP]` 的 segment id 设为 `0`，句子 B 及其 `[SEP]` 的 segment id 设为 `1`。单句输入时通常全部设为 `0`。
...
基于对比学习的模态对齐是**多模态预训练的核心方法**，其核心思想是**利用正负样本对的对比信号，将不同模态的语义表征拉到共享空间中**，典型代表是 CLIP、 ALIGN 等模型。不同模态（如图像、文本）的原始数据没有可比性，但**语义相同的内容应该有相似的表征**。基于对比学习的模态对齐采用**双编码器架构**，以最经典的**图文对齐**为例：
...
其中 $v_i \cdot t_j$ 是向量点积，$\|v_i\|、\|t_j\|$ 是向量的 L2 范数。采用 **InfoNCE 损失**（Noise Contrastive Estimation，噪声对比估计损失）作为核心损失函数
...
交叉注意力是**跨模态信息交互算子**，不是独立的损失函数。它让一种模态产生 Query，去读取另一种模态的 Key 和 Value，因此特别适合建立 token、patch、区域或音频帧之间的局部对应关系。

$$
Q_x=XW_Q,\qquad K_y=YW_K,\qquad V_y=YW_V
$$

$$
\mathrm{CrossAttention}(X,Y)=\mathrm{softmax}\left(\frac{Q_xK_y^\top}{\sqrt{d_k}}\right)V_y
$$

若 $X\in\mathbb{R}^{n_x\times d}$、$Y\in\mathbb{R}^{n_y\times d}$，则注意力权重为 `[n_x,n_y]`，输出为 `[n_x,d_v]`。例如 Q-former 用可学习 Query 通过交叉注意力从视觉特征中提取与语言相关的信息。

#### 生成式对齐

生成式对齐是多模态大模型中**高精度语义对齐**的核心技术，与对比学习驱动的“拉近距离、推远距离”的间接对齐不同，它通过**跨模态生成任务**直接强制模型学习模态间的语义映射关系，在生成过程中完成深度语义对齐，属于**强监督学习**，依赖高质量的**成对标注数据**。

##### 1 整体框架
生成式对齐不是直接让两个模态的向量相似，而是把一种模态作为条件，让模型预测另一种模态的 token 序列。以“图像生成文本”为例，可以拆成以下步骤：

1. **模态编码**：视觉编码器将图像 $I$ 转成视觉特征 $V=\mathrm{Enc}_v(I)$。若图像被切成 $N_v$ 个 Patch，则 $V\in\mathbb{R}^{N_v\times d_v}$；目标文本先经过 Tokenizer 得到 $y=[y_1,\ldots,y_T]$。
2. **模态投影与接口适配**：视觉特征维度通常和语言模型维度不同，需要通过投影层映射到语言模型空间
3. **条件注入**：常见有两种方式。

   - **Decoder-only**：将投影后的视觉 token $Z$ 插入文本嵌入序列，例如 $[Z;E(y_{<t})]$，让语言模型通过自注意力读取视觉条件。此时不一定需要单独的 Encoder-Decoder 交叉注意力模块。

   - **Encoder-Decoder**：Decoder 的 Query 来自当前文本状态，视觉表示产生 Key/Value：

$$
Q_t=H_tW_Q,\qquad K_v=ZW_K,\qquad V_v=ZW_V
$$

$$
\mathrm{CrossAttention}(H_t,Z)=\mathrm{softmax}\left(\frac{Q_tK_v^\top}{\sqrt{d_k}}\right)V_v
$$
4. **自回归预测**：模型在给定输入模态和历史 token 的条件下，预测下一个 token 的概率。若词表大小为 $Vocab$，则 logits 形状为 `[T,Vocab]`：

5. **交叉熵训练**：训练时使用真实前缀 $y_{<t}$，而不是模型自己采样的 token，优化教师强制（teacher forcing）下的负对数似然：

##### 2 优缺点分析

   1. **对齐精度高，能学习区域—文本的对应关系**
...
以**整个候选列表为训练单位**，使用列表级目标或其可优化的替代损失来改善排序指标（如 NDCG、MAP），比 Pointwise 和 Pairwise 更贴近实际排序需求。
...
先把相关性标签和模型分数分别转换成列表上的 Top-1 概率分布：

$$
P_y(i)=\frac{\exp(y_i)}{\sum_{j=1}^{n}\exp(y_j)},\qquad
P_s(i)=\frac{\exp(s_i)}{\sum_{j=1}^{n}\exp(s_j)}
$$

ListNet 的损失通常写成交叉熵；它与 $\mathrm{KL}(P_y\|P_s)$ 只相差与模型无关的常数项：

$$
\mathcal{L}_{\mathrm{ListNet}}
=-\sum_{i=1}^{n}P_y(i)\log P_s(i)
=\mathrm{KL}(P_y\|P_s)+H(P_y)
$$

- **LambdaMART 损失**

LambdaMART 是工业界主流的 Listwise 模型（基于梯度提升树），其损失是**基于 NDCG 的梯度优化**，核心是定义每个样本的 $\lambda$ 梯度（表示该样本对 NDCG 的贡献度）。对于真实相关性满足 $y_i>y_j$ 的文档对，先定义加权的 pairwise logistic 损失：

$$
\mathcal{L}_{ij}
=|\Delta\mathrm{NDCG}_{ij}|\log\left(1+\exp\left(-\sigma(s_i-s_j)\right)\right)
$$

其中 $\Delta\mathrm{NDCG}_{ij}$ 是交换文档 $i,j$ 后 NDCG 的变化量。若当前排名位置分别为 $r_i,r_j$，则：

$$
|\Delta\mathrm{NDCG}_{ij}|
=\frac{\left|\left(G(y_i)-G(y_j)\right)\left(D(r_j)-D(r_i)\right)\right|}{\mathrm{IDCG}}
$$

通常取 $G(y)=2^y-1$、$D(r)=1/\log_2(r+1)$。对 $s_i-s_j$ 求导，可得到该文档对的 Lambda 梯度：

$$
\lambda_{ij}
=-\frac{\sigma\,|\Delta\mathrm{NDCG}_{ij}|}
{1+\exp\left(\sigma(s_i-s_j)\right)}
$$

然后将所有涉及文档 $i$ 的 pairwise 梯度累加为 $\lambda_i$，并把 $\lambda_i$ 作为梯度提升树拟合的伪梯度；不同资料对 $\lambda$ 的正负号约定可能相反，但优化方向等价。
...
1. **点积的性质**​：注意力得分的计算是查询向量 Q 和键向量 K 的点积：Q⋅K。假设 Q 和 K 的每个分量是均值为 0、方差为 1 的独立随机变量，那么点积 Q⋅K 的均值是 0，而方差是 $d_k$​ （即向量维度）。
...
RoPE 的核心思想是：​**通过旋转操作将绝对位置信息注入到注意力计算中，使内积结果自然包含相对位置信息**。对于位置 $m$ 的查询向量和位置 $n$ 的键向量，RoPE 将 $d$ 维向量按每两个维度为一组进行旋转：
...
思维链（Chain of Thought, CoT）通过让模型生成显式的中间推理步骤，将复杂任务分解为子问题，显著提升复杂推理的准确性和可解释性。当模型参数量达到一定规模（通常≥10B）时，这种能力会自发涌现。

### 三种核心方法

1. **零样本 CoT（Zero-shot）**​：仅需在问题后添加"Let's think step by step"等指令，模型自动分解推理步骤。
2. **少样本 CoT（Few-shot）**​：提供 2-5 个"问题 → 推理链 → 答案"的示例，模型模仿推理模式，是最常用的方式。
3. **自一致性 CoT（Self-Consistency）**​：对同一问题生成多条推理链（Temperature>0），通过多数投票选择最一致答案，有效降低单一链的错误率。

### 进阶变体

基础 CoT 的推理链是**线性单向**的，存在错误传播、路径单一、效率偏低等局限。以下变体从不同角度优化推理过程。

#### 思维树（Tree of Thoughts, ToT）

将推理建模为**树状结构**，每个节点代表一个中间步骤候选，模型可主动探索多条路径、评估各路径优劣并**回溯修正错误**。通常配合 BFS/DFS 搜索和启发式价值函数来筛选有希望的路径。解决了线性 CoT "一步错步步错"的痛点，适合数学证明、逻辑谜题等需要探索多路径的任务。

#### 思维图（Graph of Thoughts, GoT）

将推理建模为**图结构**——节点是推理步骤，边是步骤间的依赖/关联关系，允许**跳转、循环、复用**已有推理结果，更贴近人类真实的非线性思考模式。支持多依赖、多分支的复杂推理，适合多文档综合分析、因果关系归因等需要全局视角的任务。

#### 主动提示（Active Prompting）

少样本 CoT 效果高度依赖示例与目标问题的匹配度。主动提示让模型**自主从示例池中检索或生成**最适合当前问题的示例，替代人工挑选。通过语义向量相似度匹配，大幅提升跨领域通用问答等场景下少样本 CoT 的泛化能力。

#### 链式验证（Chain Verification）

对推理链中的**每一个步骤独立验证正确性**，发现错误后仅修正该步骤及后续依赖，而非重做整个链。核心思想是定位错误 → 局部修正 → 节省重推理成本。适合数学竞赛、医疗诊断等对精度要求极高的场景。

#### 迭代式 CoT（Iterative CoT）

通过"**生成 → 评估 → 优化**"的循环不断打磨推理链：从逻辑连贯性、步骤完整性、结论合理性三个维度自我评估，找到漏洞后补充修正，多轮迭代直到无明显改进。无需外部工具或人工干预，适合创意性推理、开放式问答。

#### 链式草稿（Chain of Drafts, CoD）

生成**极简版推理链**（每步 3-5 词），在保证准确性的前提下大幅缩短输出长度，降低推理延迟和 Token 消耗。先草稿推理 → 正确则直接输出答案 → 有误则回退为完整 CoT。适合实时问答系统、边缘设备等资源受限环境。
...
Agent 与普通 LLM 调用的本质区别在于：普通调用是"一问一答"——用户给 prompt，模型回 text，结束；Agent 则将 LLM 置于一个**自主运行的执行循环**中，让它反复思考、行动、观察、修正，直到完成任务。
...
<tr>
<td>**大模型基座**<br/></td><td>决策中枢，负责理解任务、推理规划、生成行动<br/></td><td>GPT-4、Claude、Llama 等，通过 Prompt 激活推理能力<br/></td></tr>
<tr>
<td>**任务规划模块**<br/></td><td>将复杂目标拆解为可执行子任务序列<br/></td><td>CoT、ToT、ReAct 推理、动态优先级调整<br/></td></tr>
<tr>
<td>**记忆系统**<br/></td><td>存储上下文与知识，保障决策连贯性<br/></td><td>短期记忆(会话历史) + 长期记忆(向量数据库 FAISS/Milvus)<br/></td></tr>
<tr>
<td>**工具调用模块**<br/></td><td>连接外部能力，扩展 LLM 边界<br/></td><td>Function Calling、API 调用、插件系统<br/></td></tr>
<tr>
<td>**反思/反馈模块**<br/></td><td>自我评估、错误修正、优化执行路径<br/></td><td>结果校验、异常处理、迭代式规划调整<br/></td></tr>
</table>

**标准执行循环（5 步闭环）**：

```
感知 → 规划 → 行动 → 反思 → 输出 → (回到感知，进入下一轮)
```

1. **感知**：接收用户指令与环境信息（工具返回结果、记忆检索内容），构建当前轮次的 Prompt 上下文
2. **规划**：LLM 分析当前状态，判断进展，生成下一步推理/行动
3. **行动**：调用工具执行子任务（读文件、执行命令、搜索等），获取外部反馈
4. **反思**：评估工具返回结果——任务是否完成？结果是否正确？需要重试还是换方案？
5. **输出**：若任务完成则整合结果、生成最终响应；否则携带本轮信息回到步骤 1 继续循环

这个循环运转的关键在于：**每轮 LLM 的输出成为下一轮的输入**，形成链式推理——LLM 能看到自己之前的决策和工具返回结果，从而不断修正方向。

**行为模式**：根据任务复杂度和 Agent 设计，Agent 会呈现 6 种典型行为模式，实际中常混合使用：

- **反应式**：无记忆、无规划，仅根据当前输入直接输出（简单问答）
- **记忆增强型**：引入短期+长期记忆，跨会话保持连贯性（个性化助手、多轮客服）
- **目标导向型**：先拆解目标为子任务，再按序执行——**是复杂任务的核心模式**（调研报告、旅行规划）
- **协作型**：多 Agent 分工协作完成单一 Agent 无法胜任的任务（企业协同、科研辅助）
- **探索型**：面对模糊任务，通过"试错 → 学习 → 优化"迭代探索（科研假设、创意生成）
- **反射型**：执行中加入自我反思，对决策和结果评估修正（代码调试、论文润色）

### Agent Harness：Agent 的运行时外壳

**Harness**（运行时外壳）是包围在裸 LLM 外面的那层基础设施——裸 LLM 只能做"输入→输出"，Harness 将其变成可自主循环工作的 Agent。

#### 一、执行循环 (Agent Loop)

执行循环是 Harness 的心脏，本质是一个状态机驱动的 while 循环：

```
while not task_complete and iterations < max_iterations:
    1. 构建上下文 → 注入当前 task、历史消息、工具返回结果、记忆片段
    2. 调用 LLM → 模型输出"思考内容 + 下一步动作"（text 或 function_call）
    3. 解析动作 → 如果是 tool_call：提取工具名和参数；如果是 text：检查是否包含最终答案
    4. 执行工具 → 调用对应函数（读写文件、执行命令、搜索等），获取结果
    5. 观察与反思 → 工具结果追加到消息列表，模型在下一轮中判断进展、修正方向
    6. 终止判断 → 模型返回"任务完成"信号，或达到最大轮次、超时、用户中断
```

关键技术点：
- **状态传递**：每轮 LLM 的输出作为下一轮的输入上下文，形成链式推理
- **终止条件**：需要精确设计——过早终止导致任务未完成，过晚则无限循环浪费 Token
- **错误恢复**：工具执行失败时，错误信息回传给 LLM，让 LLM 自主决定重试、换工具还是放弃

#### 二、上下文管理

每轮循环的核心问题：**什么内容进入 LLM 的上下文窗口，以及如何裁剪以适应窗口限制。**

1. **消息构成**：一条标准的 Agent 消息链通常包含：
   - **System Prompt**：角色定义、行为约束、输出格式要求、工具列表及描述
   - **用户指令**：原始任务目标
   - **历史消息**：此前各轮的 LLM 输出 + 工具返回结果
   - **记忆片段**：从向量数据库检索到的相关长期记忆

2. **上下文窗口管理**：现代 LLM 的上下文窗口已到 128K-1M Token，但 Agent 循环很容易填满。Harness 需要实现裁剪策略：
   - **滑动窗口**：只保留最近 N 轮的消息，旧消息丢弃
   - **智能摘要**：将早期消息用 LLM 压缩为摘要后保留，而非直接丢弃
   - **结构化保留**：始终保留 System Prompt + 用户原始指令 + 最近的工具调用结果；中间推理过程可压缩
   - **Token 计数**：每轮追加消息前精确计算当前上下文 Token 数，触发裁剪阈值

3. **工具返回值处理**：工具输出往往冗长（如 `grep` 返回 500 行匹配结果），直接全部回传会浪费上下文。常见策略：
   - **截断**：只保留前 N 行或前 N 个字符
   - **格式优化**：将原生输出转化为 LLM 友好的结构化格式（如 JSON）
   - **分级返回**：先返回摘要，LLM 可按需请求完整内容
   - **分页**：将长输出分块，LLM 逐页查看

#### 三、工具编排 (Tool Orchestration)

工具是 Agent "动手能力"的根本。Harness 的工具有两类：

**1. 内置工具（由 Harness 直接提供）**：

- **文件系统**：读/写/编辑/搜索文件、列出目录
- **Shell 执行**：运行命令行，捕获 stdout/stderr
- **网络访问**：HTTP 请求、网页抓取
- **代码执行**：Python/JS 沙箱，支持数据分析、计算

**2. 扩展工具（通过协议接入）**：

- **MCP (Model Context Protocol)**：标准化工具接入协议，任何实现 MCP 的服务（数据库、API、第三方服务）都可以被 Agent 调用
- **Function Calling**：LLM 原生支持的 tool_use 能力，Harness 负责注册工具 schema、解析调用、执行并回传结果

**3. 工具实现细节**：

**Git 工具**——本质是对 `git` CLI 的子进程封装：

```
Harness 注册工具:
  { name: "git_diff",  描述: "查看文件变更",      参数: { staged: bool } }
  { name: "git_log",   描述: "查看提交历史",      参数: { n: int } }
  { name: "git_status",描述: "查看工作区状态",    参数: {} }
  { name: "git_add",   描述: "暂存文件",          参数: { files: string[] } }
  { name: "git_commit",描述: "创建提交",          参数: { message: string } }
  { name: "git_branch",描述: "查看/创建分支",     参数: { name?: string } }

执行流程:
  LLM 调用 git_diff({ staged: false })
  → Harness 构造命令: git diff
  → subprocess.run("git diff", cwd=workspace_dir, timeout=30s)
  → 捕获 stdout + stderr + exit_code
  → 返回: { output: "...diff 内容...", exit_code: 0 }

安全措施:
  - git push / force push / hard reset 默认需用户确认
  - git commit 需附加 --no-verify 或 --no-gpg-sign 时警告
  - 所有 git 命令限制在 workspace 目录下执行
```

**Shell/Bash 工具**——将操作系统命令作为子进程执行：

```
Harness 注册工具:
  { name: "execute_command",
    描述: "执行 Shell 命令，返回 stdout/stderr/exit_code",
    参数: { command: string, workdir?: string, timeout_ms?: number } }

执行流程:
  LLM 调用 execute_command({ command: "npm run build", workdir: "./frontend" })
  → Harness 先做安全检查:
     - 解析命令，检查是否包含禁止指令（rm -rf /, sudo, chmod 777, wget 不可信域名等）
     - 危险指令 → 抛给用户确认或直接拒绝
     - 安全指令 → 调用子进程
  → subprocess.run(command, shell=True, cwd=workdir, timeout=120s)
  → 捕获 stdout（上限 N 行，超出截断）、stderr、exit_code
  → 返回: { stdout: "...", stderr: "...", exit_code: 0 }

安全措施:
  - 命令白名单/黑名单：允许 npm/node/git/docker/ls/echo 等；禁止 rm -rf /、sudo、shutdown
  - 工作目录隔离：所有命令以项目根目录为基，禁止 cd 到系统目录
  - 超时控制：默认 120s，防止死循环命令
  - 输出截断：stdout 超过 2000 行时截断并提示，防止 500KB 输出占满上下文
```

**LSP 代码分析**——通过 Language Server Protocol 获取 IDE 级别的代码理解：

LSP 是一个 JSON-RPC 协议，定义了编辑器（Client）和语言服务器（Server）之间的通信标准。Agent Harness 作为 Client，连接语言 Server，获取结构化代码信息。

```
工作原理:
  1. Harness 启动 LSP Server 子进程（如 gopls、rust-analyzer、pylsp、typescript-language-server）
  2. 建立 JSON-RPC 通信通道（stdin/stdout 或 TCP Socket）
  3. 发送 initialize 请求 → Server 返回能力列表
  4. 发送 textDocument/didOpen 打开文件 → Server 开始索引
  5. 按需请求各类代码信息

Harness 注册的 LSP 工具:
  { "go_to_definition",     "跳转到符号定义位置",     { file, line, col } }
  { "find_references",      "查找符号的所有引用",     { file, line, col } }
  { "hover_info",           "获取符号的类型和文档",   { file, line, col } }
  { "diagnostics",          "获取文件的诊断信息（错误/警告）", { file } }
  { "document_symbols",     "列出文件中的符号表（函数、类、变量）", { file } }
  { "workspace_symbols",    "全局搜索符号",            { query } }

执行流程（以 go_to_definition 为例）:
  LLM 调用 go_to_definition({ file: "src/main.ts", line: 42, col: 10 })
  → Harness 构造 JSON-RPC: { method: "textDocument/definition", params: {...} }
  → 发送给 Language Server → 等待响应
  → Server 返回: [{ uri: "src/utils.ts", range: { start: { line: 15, col: 0 } } }]
  → Harness 解析为可读格式返回: "定义在 src/utils.ts:15"

对 Agent 的价值:
  - 不用 grep 猜符号在哪定义 → 精确跳转
  - 不用肉眼读代码判类型 → hover 直接获取类型签名
  - 不用跑编译才发现错误 → diagnostics 实时反馈语法/类型错误
  - 不用全文搜索找所有调用点 → find_references 精准定位
```

**搜索工具**——grep/glob 的 Agent 友好封装：

```
Harness 注册工具:
  { "search_content",   "用正则搜索文件内容",  { pattern, include?: "*.ts", path?: "." } }
  { "search_file",      "用 glob 模式匹配文件",  { pattern: "**/*.ts" } }

实现:
  search_content → 调用 ripgrep(rg) 或 Git Bash 的 grep 子进程
  search_file    → 调用 glob 库或 find 子进程

输出处理:
  - 匹配结果按文件分组 → 每行: filepath:lineno: content
  - 超过 N 条结果时截断并提示
  - 敏感目录（node_modules, .git, __pycache__）自动排除
```
...
```
Harness 注册工具:
  { "read_file",   "读取文件",   { path, offset?, limit?, encoding? } }
  { "write_file",  "创建/覆盖文件", { path, content } }
  { "edit_file",   "精确替换文件中的字符串", { path, old_str, new_str } }
  { "list_dir",    "列出目录内容", { path } }

关键实现细节:
  - read_file: 支持 offset/limit 分页读取，大文件不一次加载；超过 2000 行自动截断
  - edit_file: 用精确字符串替换（非行号），old_str 在文件中必须唯一匹配，否则报错
  - write_file: 写入前检查路径是否在 workspace 内，禁止往系统目录写
  - 编码处理: 自动检测 UTF-8/GBK/Latin-1，避免乱码
```

工具调用的完整流程：
```
1. Harness 将所有可用工具的定义（名称、描述、参数 schema）注入 System Prompt
2. LLM 决定调用某个工具，返回 { tool_name, parameters }
3. Harness 解析 parameters，校验参数合法性
4. Harness 执行工具，捕获返回值或异常
5. Harness 将结果格式化为消息，追加到对话历史
6. LLM 在下一轮中根据结果决定下一步
```

关键技术点：
- **并行调用**：部分 LLM 支持单轮返回多个 tool_call，Harness 需并发执行并聚合结果
- **权限控制**：未注册的工具不可调用，参数需校验类型和范围
- **重试机制**：工具调用失败（超时、网络错误）时自动重试，超过次数后回传错误给 LLM

#### 四、记忆系统

Agent 的记忆分为两个层次：

1. **短期记忆（会话级）**：当前对话的完整消息历史，以消息列表存储在内存中。Harness 需要管理消息的增删和上下文裁剪。
2. **长期记忆（跨会话级）**：
   - **存储**：用户偏好、已完成的子任务、学到的经验 → 通过 Embedding 模型向量化 → 存入向量数据库（FAISS、Milvus、Chroma）
   - **检索**：每轮循环前，用当前 query 做语义检索 → 取 Top-K 相关记忆 → 注入 System Prompt 或上下文
   - **写入时机**：会话结束时批量写入，或检测到"值得记住的信息"时即时写入
   - **管理策略**：记忆去重（避免重复存储相似内容）、遗忘机制（过期记忆降权或删除）、优先级排序

#### 五、状态管理与可恢复性

复杂任务可能运行几十甚至上百轮循环，Harness 必须支持：

- **检查点 (Checkpointing)**：定期保存完整状态（消息历史、任务进度、已打开的文件、中间产物），支持断点续传
- **序列化**：将 Agent 状态序列化为 JSON/数据库记录，支持暂停后恢复
- **分支与回溯**：类似 LangGraph 的 checkpoint，允许从某个中间状态"分叉"出新的执行路径，发现错误后回退到之前的检查点

#### 六、安全沙箱

Agent 会执行任意 Shell 命令和代码，安全是底线：

| 策略 | 说明 |
|------|------|
| **容器隔离** | 在 Docker/sandboxie 中运行 Agent，限制文件系统和网络访问 |
| **文件系统白名单** | 只允许读写指定目录，禁止访问 /etc/passwd、~/.ssh 等敏感路径 |
| **命令白名单** | 禁止 `rm -rf`、`sudo`、`chmod 777` 等危险命令 |
| **网络限制** | 限制可访问的域名/IP 范围、禁止访问内网地址 |
| **资源配额** | 限制 CPU、内存、磁盘 IO 上限，防止 Agent 耗尽系统资源 |
| **操作确认** | 高风险操作（文件删除、git push、数据库写入）需人工确认 |
| **审计日志** | 完整记录 Agent 的每一步操作，用于事后审查和安全审计 |

#### 七、典型 Harness 实现剖析

**1. Claude Code / OpenCode 类终端 Agent**：
- **循环**：接收自然语言任务 → LLM 分析 → 调用工具（读文件/搜索代码/执行命令）→ 观察结果 → 判断是否完成 → 输出结果
- **上下文管理**：保留 System Prompt + 用户指令 + 最近 N 轮工具调用结果；用 truncation 策略处理大文件读取、长命令输出
- **工具**：文件读写、glob/grep 搜索、Shell 执行、Git 操作、LSP 代码分析（见上文"工具编排"详细实现）
- **安全**：高风险命令需确认；限制 Shell 可执行的命令类型

**2. LangGraph**：
- **循环**：将 Agent 建模为有向图（Graph），节点 = 处理逻辑（LLM 调用 / 工具执行），边 = 状态转移条件
- **上下文管理**：Graph 的 State 对象承载所有上下文，每个节点读取 State → 处理 → 更新 State，天然支持精细化控制
- **工具**：兼容 LangChain 的整套工具生态，通过 ToolNode 统一管理
- **可恢复性**：内置 Checkpointer 支持断点续传

**3. AutoGPT（极简 Harness 参考）**：
- **循环**：输入目标 → LLM 生成任务列表 → 逐个执行 → 检查完成度 → 生成新任务 → 循环直到全部完成
- **上下文管理**：最简策略——保留完整历史，用 Token 上限触发截断
- **工具**：搜索引擎、文件读写、简单 HTTP 请求
- **特点**：架构简单到可以直接在 100 行代码内实现，适合学习 Harness 原理

**4. MCP (Model Context Protocol)**：
- 不是完整的 Harness，而是 Harness 的**工具层标准协议**
- 定义了统一的 Client-Server 接口：MCP Server 暴露工具列表 → MCP Client (Harness) 发现和调用
- 好处：任何实现 MCP 的工具（数据库查询、文件管理、API 网关）都能被任何兼容 MCP 的 Harness 直接使用，避免每个 Harness 都要为每个工具写适配层
...
- **ReAct**：最经典的 Agent 架构——"思考→行动→观察"三段式交错循环，推理过程透明易调试，适合复杂推理+工具组合（知识问答、代码生成）
- **Plan-and-Execute**：先全局规划再分步执行，规划与执行解耦，适合结构化任务；变种 LLMCompiler 支持最大并行化执行
- **工具增强型 RAG**：RAG 与 Agent 融合，将检索作为核心工具，解决知识过时问题（企业知识库问答、金融数据分析）
- **LATS**：强化长期规划能力，支持多轮任务间状态保持、任务依赖处理、长周期优化（项目管理、多阶段流程）

#### 多智能体架构

- **集中式控制**：中央控制器调度多个 Agent，协调简单避免冲突，但存在单点故障（管理者-执行者模式）
- **去中心化协作**：无中心节点，Agent 间消息通信自主协调，高容错可扩展（AutoGen 事件驱动）
- **分层协作**：高层战略规划+低层具体执行，兼顾全局与局部（MetaGPT 的 PM→架构师→开发→测试）
- **专家团队**：多专业 Agent 按领域分工，任务路由→结果汇总→交叉验证（CrewAI 为代表）

#### 开发框架

- **LangGraph**：以节点+边建模图状工作流，内置 Checkpointer 支持断点续传，适合多工具协作和多轮流程
- **LangChain**：一站式基础框架，组件覆盖模型调用、记忆、工具全链路，生态最丰富，入门门槛最低
- **LlamaIndex**：专注连接 LLM 与私有数据，数据索引极强，内置 RAG 全流程（加载→索引→检索→生成）
- **Semantic Kernel**：微软跨平台框架（Python/C#/Java），深度集成 Azure/M365，主打企业级安全
- **Haystack**：生产级 RAG/Agent 框架，支持分布式部署、增量索引、内置评估工具
- **DSPy**：声明式编程替代提示词调优，自动优化策略，适合高精度问答和推理任务
- **MetaGPT**：多 Agent 协作框架，模拟 PM/架构师/开发/测试分工，输出结构化文档+可运行代码

#### 编程与自动化工具

- **Claude Code**：Anthropic 终端 Agentic Coding 工具，全代码库理解+自动执行+Git 操作+MCP 扩展，Claude 4 驱动
- **OpenCode**：开源终端 Agentic Coding 工具，多后端支持（Claude/GPT/本地模型），完全开源可定制
- **OpenClaw**：开源 Computer Use Agent，操控桌面（鼠标/键盘/截图），Docker 隔离，MCP 扩展
- **Manus**：通用 AI Agent 产品，"思考→规划→执行→交付"全链路，内置多 Agent 调度+沙箱代码执行
- **AutoGPT**：开箱即用自主 Agent，输入目标自动规划分解，新手友好；但复杂任务易跑偏
