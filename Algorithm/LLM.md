...
激活函数与 FFN 结构需要区分：ReLU、GELU、SiLU/Swish 是点激活函数；GLU、SwiGLU、GeGLU 则是带门控的双分支 FFN 结构。
...
[Muon](https://github.com/KellerJordan/Muon)（MomentUm Orthogonalized by Newton-schulz）是一种主要用于神经网络**隐藏层矩阵权重**的优化器。它并不是直接把权重矩阵 $W$ 变成正交矩阵，而是先像 SGD/Momentum 一样得到梯度更新，再把这个**更新矩阵**做正交化处理。
...
官方 Muon 实现中的一组系数为 $(a,b,c)=(3.4445,-4.7750,2.0315)$，通常执行少量固定步数（例如 5 步）。迭代结束后再恢复原始转置方向，并将结果作为更新矩阵。它的优点是只需要矩阵乘法，能够在 GPU/bfloat16 环境中高效运行；代价是每一步都需要额外的矩阵乘法。
...
Muon 因此更像是“面向矩阵几何的更新规则”，而不是 AdamW 的简单替代品；在实际大模型训练中，通常需要通过实验确定哪些参数组使用 Muon。
2. 预训练目标：造就其能力的关键
...
#### 一、实现机制：如何建立跨模态联系
下面的对比学习、交叉注意力和生成式对齐描述的是“如何建立跨模态联系”。它们不是互斥分类，实际模型通常组合使用。
...
因此，交叉注意力主要回答“两个模态的哪些元素应该互相读取”；它可以作为对比学习或生成式训练中的融合模块。
...
$$
Z=\mathrm{Proj}(V)=VW_p+b_p,\qquad Z\in\mathbb{R}^{N_v\times d_{model}}
$$

其中 $W_p\in\mathbb{R}^{d_v\times d_{model}}$。在 Q-former 等结构中，$\mathrm{Proj}$ 还可能包含可学习 Query 和交叉注意力，用于压缩视觉 token 并筛选与语言相关的信息。
3. **条件注入**：常见有两种方式。
...
$$
P(y\mid I)=\prod_{t=1}^{T}P(y_t\mid I,y_{<t})
$$

5. **交叉熵训练**：训练时使用真实前缀 $y_{<t}$，而不是模型自己采样的 token，优化教师强制（teacher forcing）下的负对数似然：

$$
\mathcal{L}_{gen}=-\sum_{t=1}^{T}\log P(y_t\mid I,y_{<t})
$$

这样，模型要想降低损失，就必须同时利用视觉条件和语言上下文生成语义正确的文本；这比只判断图文是否匹配施加了更强的跨模态约束。

6. **训练策略**：工程上常见“冻结视觉编码器、先训练投影层/Query 模块，再联合微调语言模型”的分阶段流程，也可以在足够数据和算力下联合训练全部模块。不同阶段的参数是否冻结，决定了训练成本、视觉特征可塑性和灾难性遗忘风险。

###### 3.2 优缺点分析
   优点

   1. **对齐精度高，能学习区域—文本的对应关系**
      生成任务要求模型捕捉模态间的语义对应关系，例如图像区域与文本词汇之间的关联；生成结果本身也会反过来检验条件信息是否被正确理解。
   2. **直接支持复杂下游任务**
   ...
   5. **计算成本高，训练难度大**
      编码器-解码器或 Decoder-only 架构通常比双编码器包含更多交互计算，且自回归生成需要逐 token 解码；训练大规模图文生成模型也需要较高的算力和高质量配对数据。
   6. **容易出现“语义漂移”**
...
| 目标 | 更直接的机制 | 典型结构 |
| --- | --- | --- |
| 图文检索、零样本分类 | 对比学习 | 双编码器 |
| 区域—短语、帧—文本对应 | 交叉注意力 + 特征匹配 | 跨模态融合模块 / Q-former |
| VQA、图像描述、多模态对话 | 生成式训练，可结合交叉注意力 | Encoder-Decoder 或 Decoder-only |

这里的区分是：对比学习和生成式目标属于训练信号，交叉注意力属于融合算子。
