...
因此，交叉注意力主要回答“两个模态的哪些局部元素应该互相读取”；它可以作为对比学习、生成式训练或端到端训练中的融合模块。
...
端到端不是与对比学习、交叉注意力并列的单一对齐损失，而是一种训练和架构组织方式：把多模态 token 放入同一个模型，让下游任务损失反向传播，联合学习表征、融合和生成。它可以使用混合自注意力，也可以在模型内部使用交叉注意力。

一种典型实现是**单 Transformer 编码器 + 多模态 Token 化层 + 下游任务头**，但实际模型也可以保留视觉编码器、投影层或交叉注意力模块；关键标准是对齐相关模块与任务目标一起训练，而不是先单独完成一个固定的对齐阶段。
...
2. **跨模态交互更直接**：混合注意力让文本的每个 word 和图像的每个 patch 都能直接交互，比生成式的“解码器 → 编码器”跨注意力更直接；
...
| 希望融合与任务目标共同优化 | 端到端训练 | 混合 token Transformer 或联合多模块架构 |

这里的区分是：对比学习和生成式目标属于训练信号，交叉注意力属于融合算子，端到端属于训练组织方式。
